...
<div class="container">
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form action="{{ route('pinjam.update', $pinjam->id) }}" method="POST" class="w-100">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="anggota_id" class="form-select @error('anggota_id') is-invalid @enderror" required>
                            @foreach($anggota as $a)
                                <option value="{{ $a->id }}" {{ old('anggota_id', $pinjam->anggota_id) == $a->id ? 'selected' : '' }}>
                                    {{ $a->nama }} ({{ $a->nim }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="buku_id" class="form-select @error('buku_id') is-invalid @enderror" required>
                            @foreach($buku as $b)
                                <option value="{{ $b->id }}" {{ old('buku_id', $pinjam->buku_id) == $b->id ? 'selected' : '' }}>
                                    {{ $b->judul }} - {{ $b->penulis }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="tgl_pinjam" value="{{ old('tgl_pinjam', $pinjam->tgl_pinjam) }}" 
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="tgl_kembali" value="{{ old('tgl_kembali', $pinjam->tgl_kembali) }}" 
